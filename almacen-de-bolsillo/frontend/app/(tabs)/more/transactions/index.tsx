      ...
      {transactions.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item: transaction }) => {
            ...
            const directionBadgeClass = isIncome ? "bg-emerald-50 dark:bg-emerald-950/60" : "bg-red-50 dark:bg-red-950/60";
            const directionTextClass = isIncome ? "text-emerald-700 dark:text-emerald-300" : "text-red-700 dark:text-red-300";
            ...
            return (
              <View className="rounded-3xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
                <View className="flex-row items-start justify-between gap-3">
                  <View className="flex-1">
                    <Text className="text-2xl font-black text-slate-950 dark:text-white">Movimiento #{transaction.id}</Text>
                ...
                <View className="mt-4 flex-row flex-wrap gap-2">
                  ...
                  <View className={`rounded-full px-3 py-1.5 ${directionBadgeClass}`}>
                    <Text className={`text-xs font-bold ${directionTextClass}`}>
                  ...
                  <View className="rounded-full bg-slate-100 px-3 py-1.5 dark:bg-slate-900">
                    <Text className="text-xs font-bold text-slate-700 dark:text-slate-200">
                      {new Date(transaction.createdAt).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })}
                    </Text>
                  </View>
                </View>
              </View>
