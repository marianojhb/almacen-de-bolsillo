...
import type { PurchaseOrderWithItemsDto } from "@almacen/shared";
import { FlatList, Text, View } from "react-native";
import { getPurchaseById } from "@/services/purchasesApi";

export default function PurchaseDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const [purchase, setPurchase] = useState<PurchaseOrderWithItemsDto | null>(null);
  ...
  return (
    <View className="flex-1 p-4">
      <Stack.Screen options={{ title: `Compra Nº${purchase.id}` }} />

      <View className="mb-4 rounded-lg border border-gray-300 p-4 dark:border-gray-700">
        <Text className="text-2xl font-bold dark:text-white">Compra Nº{purchase.id}</Text>
        <Text className="mt-2 text-base dark:text-white">
          Total: {purchase.total.toLocaleString("es-AR", { style: "currency", currency: "ARS" })}
        </Text>
        <Text className="mt-1 text-base dark:text-white">
          Fecha: {new Date(purchase.createdAt).toLocaleString("es-AR")}
        </Text>
        <Text className="mt-1 text-base dark:text-white">Proveedor Nº{purchase.supplierId}</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="rounded-lg border border-gray-300 p-3 dark:border-gray-700">
            <Text className="font-semibold dark:text-white">Producto Nº{item.productId}</Text>
            <Text className="dark:text-white">Cantidad: {item.quantity}</Text>
            <Text className="dark:text-white">
              Subtotal: {item.subtotal.toLocaleString("es-AR", { style: "currency", currency: "ARS" })}
            </Text>
          </View>
        )}
      />
